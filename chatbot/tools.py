    ...
    def get_medication_logs() -> str:
        ...
        if isinstance(logs, list) and len(logs) > 0:
            ...
            for idx, log in enumerate(logs, 1):
                ...
                unit=log.get('unit','N/A')
                customTimes=log.get('customTimes','N/A')
                intervalDays=log.get('intervalDays','N/A')
                intervalStartDate=log.get('intervalStartDate','N/A')
                lasttaken=log.get('lastTakenDate','N/A')
                ispriority=log.get('isPriority','N/A')
                nextScheduledDate=log.get('nextScheduledDate','N/A')
                takentoday=log.get('takenToday','N/A')
                dosage = log.get('dose', 'N/A')
                freq = log.get('frequency', 'N/A')
                timeofday = log.get('timesOfDay', 'N/A')
